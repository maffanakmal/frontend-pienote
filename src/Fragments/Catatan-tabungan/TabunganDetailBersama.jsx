...
const TabunganDetailBersama = () => {
    const { groupsavings_id } = useParams(); // Get groupsavings_id from URL params
        ...
        axios.get(`http://localhost:5081/detailtabunganbersama/${groupsavings_id}`, { withCredentials: true })
            .then(res => {
                ...
                setValues({
                    ...
                    target,
                    place_saving,
                    dateline, // Format date for input type date
                    notification,
                    wishlist
                });
            })
            .catch(err => {
                console.error('Error fetching group savings:', err);
        ...
        axios.post(`http://localhost:5081/detailtabunganbersama/${groupsavings_id}`, values, { withCredentials: true })
            .then(res => {
                if (res.data.error) {
                    ...
                    setSuccess('group saving has been updated!');
                    setShow(true);
    ...
    return (
        <>
            <ProfileNavbarComp />
            <Container className="tabunganbersama-container min-vh-100 d-flex align-items-center justify-content-center" style={{ paddingTop: '100px' }}>
                    ...
                    <Form onSubmit={handleSubmit}>
                        ...
                        <Row className='splitbill-input-row mt-3 p-4'>
                            <Col md='12'>
                                ...
                                <Form.Group className="mb-3">
                                    <Form.Label>Teman Tabungan</Form.Label>
                                    {Array.isArray(values.friends) && values.friends.map((friend, index) => (
                                        <div key={index} className="d-flex mb-2 align-items-center">
                                            ...
                                            <Button
                                                ...
                                                className="ms-2"
                                            >
                                                <i className="fa-solid fa-trash"></i> {/* Ensure the FontAwesome icon is correctly used */}
                                            </Button>
                                        </div>
                                    ))}
                                </Form.Group>
...
export default TabunganDetailBersama;
